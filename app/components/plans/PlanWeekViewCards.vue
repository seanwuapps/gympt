  ...
  <div class="plan-week-view-cards">
    <ul class="week-schedule">
      ...
      >
        ...
        <div v-if="editable" class="actions-column">
          <BaseButton
    ...
    >
      ...
      <template #footer>
        <BaseButton label="Cancel" @click="closeEditDialog" text />
        <BaseButton label="Save" @click="saveChanges" :disabled="!canSave" :loading="saving" />
      </template>
    </BaseDialog>
  </div>
</template>
...
async function openEditDialog(day: string, modality: string) {
  ...
  try {
    const weekKey = `week${props.initialWeek}`
