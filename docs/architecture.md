...
## 1. Overview
Nuxt 4 application (Vue 3 + TS) with Element Plus UI, Supabase (Postgres + Auth), Drizzle ORM, and Cloudflare Workers AI via an OpenAI-compatible API. Server routes in Nuxt handle AI calls and data mutations securely.
...
- `pages/` — onboarding, sessions, reports
- `components/` — session runner, feedback
- `composables/` — `useSessionRunner.ts`
- `server/api/` — `ai/session.generate.post.ts`, `ai/exercise.substitute.post.ts`, `adapt/next-session.post.ts`
